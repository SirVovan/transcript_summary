...
# Фазы 2-6 — раскрытие при выполнении

Фазы 2-6 содержат ~21 задачу той же структуры (TDD: failing test → реализация → проходит → commit). **Каждая задача раскрывается по тому же шаблону, что Фазы 0-1, прямо перед выполнением соответствующей фазы**, не заранее.

## Почему так

- Полное раскрытие всех 6 фаз сразу удвоило бы размер плана до ~5000 строк.
- К моменту реализации Фазы N мы уже будем знать поправки и нюансы из Фаз < N — они попадут в раскрытие Фазы N автоматически.
- Это снижает риск переписывания плана задним числом.

## Процедура раскрытия (для orchestrator-а)

Когда orchestrator завершает фазу N и доходит до чекпоинта, **дальше не идёт автоматически**. Ждёт явной команды Owner-а в чате.

Owner пишет в чат **одно сообщение**:

```
Раскрой Фазу N+1 и продолжай выполнение.
```

После этого orchestrator (Opus 4.8 в новой сессии) делает:

1. **Прочитать Фазы 0-1** как образец стиля: точные пути файлов, TDD-шаги, готовый код в каждом шаге, конкретные команды pytest, отдельный commit на каждую задачу.
2. **Раскрыть нужную фазу прямо в этом файле** — дописать полные TDD-блоки для каждой задачи (заменить «свёрнутый» список ниже на полные разделы вида `## Task N.M: ...`).
3. **Закоммитить раскрытие**: `docs(plans): раскрыта Фаза N+1`. Отдельный коммит от реализации.
4. **Продолжить выполнение** задач этой фазы через subagent-driven flow (см. execution-distribution).
5. По завершении фазы — снова остановиться на чекпоинте, ждать команду на следующую.

## Что должно содержать раскрытие каждой задачи

Жёсткие требования (как в Фазах 0-1):

- **Files:** конкретные пути с пометкой Create / Modify / Test.
- **Каждый шаг — отдельный чекбокс** в формате `- [ ] **Step N: ...**`.
- **TDD-цикл**: failing test → запустить и убедиться что падает → реализация → запустить и убедиться что проходит → commit.
- **Полный код** в шаге test и шаге implementation, без сокращений «как в задаче M».
- **Точные команды** запуска тестов с ожидаемым результатом.
- **Готовое сообщение коммита** в шаге commit.

Запрещено:
- «TBD», «TODO», «implement later».
- «Аналогично задаче N.M» вместо полного кода.
- Ссылки на типы/функции, не определённые в этом же или предыдущем шаге.

## Команды для Owner-а — шесть штук на весь проект

После каждой завершённой фазы — одна команда в чат:

| После | Команда |
|---|---|
| Чекпоинт 1 (текущий конец раскрытого плана) | `Раскрой Фазу 2 и продолжай выполнение.` |
...
Если хочешь остановиться раньше — просто не отправляй следующую команду. Orchestrator не должен идти дальше без явного сигнала.

---

## Краткое содержание оставшихся фаз (для оркестратор-а — основа для раскрытия)
