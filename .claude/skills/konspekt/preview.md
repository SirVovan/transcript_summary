...
Формула: убрать у имени транскрипта любой префикс `SRC_…_` (`SRC_transcript_`, `SRC_Тайминг_`, или просто `SRC_`) и расширение → получится `<название>`.

- В **рабочей папке урока** имя обзора — `OUT_<название>_обзор.md` (префикс `OUT_` обязателен, симметрично с `OUT_<название>_мастер.md`).
- В `transcripts/` проекта — без `OUT_`: `<название>_обзор.md`.
