...
- Базируется на имени транскрипта: убрать любой префикс `SRC_…_` (например, `SRC_transcript_`, `SRC_Тайминг_`, или просто `SRC_`), убрать расширение, добавить `_обзор.md`.
- Если транскрипт уже без префикса `SRC_` — просто заменить расширение на `_обзор.md`.
