...
Использование:
  1. cp recon_patch_template.py patch_recon_<урок>.py
  2. Заполнить все TODO-секции.
  3. python patch_recon_<урок>.py

Референсы:
  - patch_recon_m2u6.py  — параллельные агенты + критики (форма SVG: ветви → файл)
  - patch_recon_m2u7.py  — цепочка артефактов с правками (форма SVG: горизонтальная цепь)
  - layer3_recon.md       — полная инструкция по Слою 3
"""
