...
ШАГ 0: Оценка размера (preprocessor.py info)
    ↓
    ├─ если > 25K токенов → Предобработка (нарезка на части)
    │                             ↓
    └─ если ≤ 25K токенов ──────→ ┤
                                   ↓
...
4. Каждый chunk проходит ШАГ 1а → 1б → 1в независимо.
   ...
   - Для каждого chunk: взять готовый мастер-MD (результат ШАГ 1в).
   - Убрать из каждого chunk-MD дублирующийся заголовок (кроме первого).
   - Бригадир финального прохода: прочитать все chunk-MD подряд, написать переход на стыке частей (1–2 предложения), сохранить единый мастер-MD.
   *(Детальный дизайн для edge-cases — отдельный спек; базовый случай покрыт выше.)*
...
Прочитать `subagent_writer.md` и передать его содержимое каждому subagent'у в начале промпта.
